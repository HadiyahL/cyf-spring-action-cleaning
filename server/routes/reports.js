...
router.get(
	...
	(req, res, next) => {
		...
		db.query(
			`SELECT j.visit_on, c.name column_1, b.address column_2, (j.end_time - j.start_time) duration 
			FROM jobs j
			INNER JOIN workers w ON j.worker_id=w.id
			INNER JOIN branches b ON j.branch_id=b.id
			INNER JOIN customers c ON j.customer_id=c.id
...
router.get(
	...
	(req, res, next) => {
		...
		db.query(
			`SELECT j.visit_on, b.address column_1, w.name column_2, (j.end_time - j.start_time) duration
			FROM jobs j
			INNER JOIN workers w ON j.worker_id=w.id
			INNER JOIN branches b ON j.branch_id=b.id
			INNER JOIN customers c ON j.customer_id=c.id
